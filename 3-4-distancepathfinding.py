...
#
# Input 1: distance image
# Input 2: Navigable space (ImageData with navigable space = 1, non-navigable space = 0)
...
def ShortestPathGrid(voxels, start, end, distanceArr):
    path_found = False
...
distArr = self.GetInputDataObject(0, 0)
...
for z in range(dims[2]):
    for y in range(dims[1]):
        for x in range(dims[0]):
            pos = (x,y,z)
            distance = GetArrValue(distArr, dims, pos)
...
path = ShortestPathGrid(navigable_space, start, end, distanceVolume)
