...
def ShortestPathGrid(voxels, start, end, distArr, dims):
    path_found = False
        ...
        for z_r in range(-vertical_footspan, vertical_footspan+1):
            ...
            for y_r in range(-1,2):
                ...
                for x_r in range(-1,2):
                    x_n = current[0] + x_r    
                    next = (x_n,y_n,z_n)
                    distance = GetArrValue(distArr, dims, next)
                       
                    if x_r != 0 and y_r != 0:
                        new_cost = cost_so_far[current] + 4 + abs(z_r) + distance
                    else:
                        new_cost = cost_so_far[current] + 3 + abs(z_r) + distance
...
########### Read inputs ##########
##################################
dilate = self.GetInputDataObject(0, 0)
distArr = self.GetInputDataObject(0, 1)
idi = self.GetInputDataObject(0, 1)
#idi = vtk.vtkTypeUInt32Array()
#idi.DeepCopy(distArr)
startLocation = self.GetInputDataObject(0, 2)
endLocation = self.GetInputDataObject(0, 3)

#dims = dilate.GetDimensions()
dims = dilate.GetDimensions()

distArr = distArr.GetPointData().GetScalars()
...
for z in range(dims[2]):
    for y in range(dims[1]):
        for x in range(dims[0]):
            if GetArrValue(arr, dims, (x, y, z)) == 10000:
                SetArrValue(arr, dims, (x, y, z), 0)
            else:
                SetArrValue(arr, dims, (x, y, z), 1)
...
path = ShortestPathGrid(navigable_space, start, end, distArr, dims)
